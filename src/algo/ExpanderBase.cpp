...
void add_expander_options(Processor* p) {
    p->add_gopt("batch", "batch size for pair aligner",
                "EXPANDER_BATCH");
    p->add_gopt("gap-range", "Max distance from main diagonal of "
                "considered states of pair alignment. "
                "The more gap_range, the more time.",
                "ALIGNER_GAP_RANGE");
    p->add_gopt("max-errors",
                "Max number of errors in pair alignment",
                "ALIGNER_MAX_ERRORS");
    p->add_gopt("gap-penalty", "Gap open or extension penalty",
                "ALIGNER_GAP_PENALTY");
    p->add_gopt("mismatch-penalty", "Mismatch penalty",
