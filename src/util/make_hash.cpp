...
hash_t make_hash(const char* start, size_t length, int ori) {
    hash_t result = 0;
    for (int j = 0; j < length; j++) {
        const char* i = start + j * ori;
        hash_t value = char_to_size(ori == 1 ? *i : complement(*i));
...
hash_t reuse_hash(hash_t old_hash, size_t length,
    ...
    int occupied = std::min(POS_BITS * length, BYTE_BITS * sizeof(hash_t));
    if (forward) {
