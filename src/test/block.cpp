...
BOOST_AUTO_TEST_CASE (Block_slice_1) {
    using namespace npge;
    SequencePtr s1 = boost::make_shared<InMemorySequence>("TAGTCCG-");
    SequencePtr s2 = boost::make_shared<InMemorySequence>("TGTT-CG-");
    SequencePtr s3 = boost::make_shared<InMemorySequence>("TG---CG-");
    ...
    new MapAlignmentRow("TG---CG-", f3);
    b.insert(f3);
    boost::scoped_ptr<Block> b11((b.slice(1, 1)));
    BOOST_REQUIRE(b11->size() == 3);
    BOOST_CHECK(b11->alignment_length() == 1);
    Strings ff;
    BOOST_FOREACH (Fragment* f, *b11) {
        ff.push_back(f->str());
    }
    std::sort(ff.begin(), ff.end());
    BOOST_CHECK(ff[0] == "A");
...
BOOST_AUTO_TEST_CASE (Block_slice_empty_subfragment) {
    using namespace npge;
    SequencePtr s1 = boost::make_shared<InMemorySequence>("TAGTCCG-");
    SequencePtr s2 = boost::make_shared<InMemorySequence>("TGTT-CG-");
    SequencePtr s3 = boost::make_shared<InMemorySequence>("TG---CG-");
    ...
    std::sort(ff.begin(), ff.end());
    BOOST_CHECK(ff[0] == "GTC");
    BOOST_CHECK(ff[1] == "TT-");
}

BOOST_AUTO_TEST_CASE (Block_slice_empty_subfragment_2) {
    using namespace npge;
    SequencePtr s1 = boost::make_shared<InMemorySequence>("TAGTCCG-");
    SequencePtr s2 = boost::make_shared<InMemorySequence>("TGTT-CG-");
    SequencePtr s3 = boost::make_shared<InMemorySequence>("TG---CG-");
    ...
    new MapAlignmentRow("TG---CG-", f3);
    b.insert(f3);
    boost::scoped_ptr<Block> b22((b.slice(2, 2)));
    BOOST_REQUIRE(b22->size() == 2);
    BOOST_CHECK(b22->alignment_length() == 1);
    Strings ff;
    BOOST_FOREACH (Fragment* f, *b22) {
        ff.push_back(f->str());
    }
    std::sort(ff.begin(), ff.end());
    BOOST_CHECK(ff[0] == "G");
...
BOOST_AUTO_TEST_CASE (Block_identity) {
    ...
    BOOST_CHECK(my_abs(b2->identity() - D(4.5) / D(6)) <
                D(0.01));
    Block* b3 = new Block();
